...
# setup_remote <target> <url_default> <secret_prefix>
setup_remote() {
  local target="$1" url_default="$2" prefix="$3"
  local url anon svc
  ...
  if [ -z "$anon" ]; then
    log "ERROR: SUPABASE_TARGET=$target but ${prefix}_SUPABASE_ANON_KEY is not set."
    log "Add ${prefix}_SUPABASE_ANON_KEY (and optionally ${prefix}_SUPABASE_URL /"
    log "${prefix}_SUPABASE_SERVICE_ROLE_KEY) in the Cloud Agent Secrets panel, then re-run."
    return 1
  fi
...
case "$TARGET" in
  local)
    ...
    ;;
  staging|stg)
    setup_remote staging "$STAGING_URL_DEFAULT" STAGING
    ;;
  prod|production)
    setup_remote prod "$PROD_URL_DEFAULT" PROD
    ;;
  *)
