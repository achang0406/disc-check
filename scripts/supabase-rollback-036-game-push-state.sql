-- Rollback Phase 2b-ii: drop headcount triggers and game_push_state.
...
DROP FUNCTION IF EXISTS maintain_rsvp_push_headcount();
DROP FUNCTION IF EXISTS sync_game_push_state_denorm();
...
-- Restore admin_upsert_game without push-state upsert on insert.
...
DROP FUNCTION IF EXISTS upsert_game_push_state_for_cycle(TEXT, TIMESTAMPTZ);
