-- Max 7 games per group; one game per weekday per group.
...
BEGIN
  ...
  IF v_dupes > 0 THEN
    RAISE EXCEPTION
      'Cannot add games_group_weekday_unique: % duplicate group/weekday pairs exist. Resolve manually before migrating.',
      v_dupes;
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT v_is_update THEN
    ...
    IF v_game_count >= 7 THEN
      RAISE EXCEPTION 'group game limit reached';
    END IF;
  END IF;
  ...
  EXCEPTION
    WHEN unique_violation THEN
      RAISE EXCEPTION 'this group already has a game on that day of the week';
  END;
END;
$$;
